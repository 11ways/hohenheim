...
import be.elevenways.zenit.cms.server.page.SettingsLabels;
 ...
 * group/order/description stay where every other hohenheim peer declares them. Everything
 * else is the framework's standard shape (SettingsPage.DEFAULT_SLUG, SettingsLabels.page(),
 * Icon.SETTINGS); SettingsPage.standard(...) cannot be used as-is only because it offers no
 * nav-group seam.
 */
public final class AdminSettingsPage extends SettingsPage {

    public AdminSettingsPage(@NonNull List<Mount> mounts) {
        super(Identifier.of("hohenheim", "settings"), SettingsPage.DEFAULT_SLUG,
            SettingsLabels.page(), Icon.SETTINGS, mounts);
    }
