...
Swapped to hohenheim `ab89d70a` (jar sha256 `4ebfcec7cc5f3839...`, stamp 13/13 `dirty=false`, same chain as wave 5) with `tools/deploy-host.sh kuifje <jar>` in one run, exit 0; preflight `/root/hohenheim-preflight-20260902-110437/`, rollback jar `78e6cfac`; no migration (top M009). Healthy after both restarts, `/health` 200 from outside. Ships the shared-engine credential fix (taken logical name/user refused, move renames, tenant names namespaced).
ROLLBACK: `tools/deploy-host.sh --rollback kuifje --preflight /root/hohenheim-preflight-20260902-110437`.
...
## Deploy 2026-09-03 (kuifje): 1687be16, the list-card filter bar and the 2026-09-02 chain wave
...
(cms.js +364 KB raw). Deployed with `zenit-dev deploy kuifje --pushed` (`tools/deploy-host.sh`).
Preflight `/root/hohenheim-preflight-20260903-211900/` (`hohenheim.db.pre` integrity ok,
...
Healthy after both restarts; `zenit-dev deployed kuifje` = current.
Note: the preflight step printed `ls: cannot access '/root/.../settings': Permission denied` and
"settings: 0 files copied" -- the count runs as the ssh user while the copy runs under sudo, so the number is
unreadable here, not necessarily zero; check the directory as root before trusting a rollback of settings.
ROLLBACK: `tools/deploy-host.sh --rollback kuifje --preflight /root/hohenheim-preflight-20260903-211900`.


## Deploy 2026-09-05 (kuifje): compact CMS lists and visible range labels
...
`zd_deploy target:kuifje` with the same artifact used on all three hosts.
...
Preflight: `/root/hohenheim-preflight-20260905-114812/`. Database backup integrity
...
Rollback: `tools/deploy-host.sh --rollback kuifje --preflight
/root/hohenheim-preflight-20260905-114812` (one command). The retained rollback jar
...
## Deploy 2026-09-05 (kuifje): date/time filters with translated comparisons
...
Preflight: `/root/hohenheim-preflight-20260905-132942/`. Database integrity,
...
health checks. Retained rollback jar SHA-256: `1cee05c28385e4516f1e11b7ba05743278c3025c38837e7f6807e161d89c4a98`.
Rollback command: `tools/deploy-host.sh --rollback kuifje --preflight /root/hohenheim-preflight-20260905-132942`.
